import { getMenuItemPath, type MenuItem } from "../content/siteContent";
  ...
  return (
    ...
    >
        ...
        >
          <img
            className={`h-full w-full object-cover contrast-[1.04]
              saturate-[1.08] transition-transform duration-300
              hover:scale-[1.03]`}
            src={item.image}
      ...
      <div className="px-[0.95rem] pt-[0.9rem] pb-[0.86rem] text-center">
        <h3
          className={`grid min-h-[2.3rem] place-items-center
            [font-family:Impact,Haettenschweiler,'Arial_Narrow_Bold',sans-serif]
            text-[clamp(1rem,1.25vw,1.2rem)] leading-[1] text-balance
        ...
        >
          {typeof item.price === "string" ? (
            <strong
              className={`block text-[clamp(1.9rem,2.55vw,2.35rem)]
                leading-[0.9]`}
            >
              {item.price}
            </strong>
            ...
            <div
              className={`divide-brand-yellow/25 grid grid-cols-2
                divide-x`}
            >
              {item.price.map((option) => (
                ...
                >
                  <span
                    className={`text-brand-white/65 block font-sans
                      text-[0.58rem] leading-none font-black
                  ...
                  <strong
                    className={`mt-1 block
                      text-[clamp(1.35rem,2vw,1.8rem)] leading-none`}
                  >
                    {option.value}
                  </strong>
                </div>
